...
class BranchesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create branch' do
    assert_difference('Branch.count') do
      post branches_url, params: { branch: { location: @branch.location, store_id: @branch.store_id } }
    end
  ...
  test 'should update branch' do
    patch branch_url(@branch), params: { branch: { location: @branch.location, store_id: @branch.store_id } }
