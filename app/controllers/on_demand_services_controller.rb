...
class OnDemandServicesController < ApplicationController
  ...
  def on_demand_service_params
    params.require(:on_demand_service).permit(:service_provider_id, :name, :description, :service_type,
                                              :service_category, :service_sub_category, :duration, :duration_unit, :distance, :distance_unit, :price, :discount, :tax)
  end
end
